...
//
public class STPlan implements Comparable<STPlan>
   {
   //
   ...
   //
   public STPlanNode planTreeRoot;
   private SlowSparseNumberedGraph<STPlanNode> planTree;
   ...
   //
   public SlowSparseNumberedGraph<STPlanNode> getPlanTree()
      {
      return planTree;
      }
   ...
   //
   public HashSet<Variable> isCompatible(STPlan otherPlan)
      {
   ...
   //
   public HashSet<Variable> makeUnionCompatible(STPlan otherPlan)
      {
   ...
   // Returns the union of all variables in this and the otherPlan
   //
   public HashSet<Variable> makeMinusCompatible(STPlan otherPlan)
      {
      HashSet<Variable> unionVariableSet = new HashSet<Variable>(this.planTreeRoot.getProducedVariables());
      unionVariableSet.addAll(this.requiredVariables);
   ...
   //
   public boolean sameRequirement(STPlan otherPlan)
      {
   ...
   public void makePlanOptional(Pattern optionalPattern)
      {
      STEPlanNodeType newRootType = STEPlanNodeType.LEFT;
      STPlanNode newRoot = new STPlanNode(newRootType, new HashSet<Variable>(), optionalPattern);
   ...
   public void makePlanSubselect(Pattern subselectPattern)
      ...
      STEPlanNodeType newRootType=STEPlanNodeType.SUBSELECT;
      STPlanNode newRoot = new STPlanNode(newRootType, operatorVariables, subselectPattern);
   ...
   //
   public int compareTo(STPlan arg0)
      {
   ...
   public String toString()
      ...
      int counter = 0;
      Map<STPlanNode,Integer> nodeIds = HashMapFactory.make();
      for(STPlanNode n : planTree) {
    	  nodeIds.put(n, counter++);
      }
      
      for(STPlanNode n : planTree) {
    	  returnString.append("node: " + nodeIds.get(n) + "\n");
    	  STPlanNode p = n.getPredecessor(this);
    	  if (p != null) {
    		  returnString.append("logical predecessor: " + nodeIds.get(p) + "\n");
    	  }
    	  returnString.append(n.toString() + "\n");
    	  for(Iterator<STPlanNode> succs = planTree.getSuccNodes(n); succs.hasNext(); ) {
   ...
   public boolean isVariableOfSingleType(Store store, Variable v) {
	   ...
	   Iterator<STPlanNode> it = planTree.getNodeManager().iterator();
	   Set<Db2Type> types = HashSetFactory.make();
	   
	   while (it.hasNext()) {
		   STPlanNode n = it.next();
		   if (n.getProducedVariables().contains(v) && n.getType() == STEPlanNodeType.TRIPLE) {
...
public static STAccessMethod getSTAccessMethod(QueryTriple triple,
		SPARQLOptimizerStatistics stats, QueryTripleNode n, Query q, PredicateTable revPreds) {
	STAccessMethod am = null;
	...
	case QueryTripleNode.DPH: {
		if (QueryTripleNode.isNotBoundVariable(n.getQueryTriple().getSubject(), n.getFilterBindings())) { // subject
			// is a
			// variable
			am = new STAccessMethod(
					STEAccessMethodType.DPH_POLL_SUBJECT,
					new com.ibm.rdf.store.schema.Pair<Double>(
			...
			am = new STAccessMethod(
					STEAccessMethodType.DPH_INDEX_SUBJECT,
					new com.ibm.rdf.store.schema.Pair<Double>(
							stats.globalStatistics.getSubjectStatistic()
									.average(),
							stats.globalStatistics.getSubjectStatistic().stdev()));
	...
	case QueryTripleNode.RPH: {
		if (QueryTripleNode.isNotBoundVariable(n.getQueryTriple().getObject(), n.getFilterBindings())) { // object
			...
			// variable
			am = new STAccessMethod(
					STEAccessMethodType.RPH_POLL_OBJECT,
					new com.ibm.rdf.store.schema.Pair<Double>(
							stats.globalStatistics.getSubjectStatistic()
									.average()
									* stats.globalStatistics.getObjectStatistic()
			...
			am = new STAccessMethod(STEAccessMethodType.RPH_INDEX_OBJECT,
					new com.ibm.rdf.store.schema.Pair<Double>(
							stats.globalStatistics.getObjectStatistic()
									.average(),
							stats.globalStatistics.getObjectStatistic().stdev()));
	...
	case QueryTripleNode.GDPH: {
		if (n.getPattern().getGraphRestriction() != null
			...
			// variable
			am = new STAccessMethod(
					STEAccessMethodType.DPH_POLL_GRAPH,
					new com.ibm.rdf.store.schema.Pair<Double>(
			...
			am = new STAccessMethod(STEAccessMethodType.DPH_INDEX_GRAPH,
					new com.ibm.rdf.store.schema.Pair<Double>(2
							* stats.perGraphSingleStatistic.getGraphStatistic()
									.average()
			...
			am = new STAccessMethod(
					STEAccessMethodType.DPH_INDEX_GRAPH,
					new com.ibm.rdf.store.schema.Pair<Double>(
							2 * stats.perGraphSingleStatistic.getGraphStatistic()
									.average(),
							stats.perGraphSingleStatistic.getGraphStatistic()
	...
	case QueryTripleNode.SCAN:
		...
		am = new STAccessMethod(revOneToOne || (needObject&&!needSubject)? STEAccessMethodType.RPH_SCAN: STEAccessMethodType.DPH_SCAN,
				new com.ibm.rdf.store.schema.Pair<Double>(Double.MAX_VALUE,
						Double.MAX_VALUE));
		break;
	}
	return am;
