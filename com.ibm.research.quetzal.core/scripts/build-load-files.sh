...
gawk -b '{ match($0, /^(.*[^[:blank:]])[[:blank:]]+([0-9]+)$/, a); print substr($0, a[2, "start"], a[2, "length"]) "\t" substr($0, a[1, "start"], a[1, "length"]); }' $NT_FILE_1.types > $LOAD_DIR_1/datatypes.load
